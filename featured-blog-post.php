...
// Featured News Blog Module
class DS_Custom_Featured_Blog extends ET_Builder_Module {
    function init() {
        $this->name = esc_html__( 'Blog - Featured Header', 'et_builder' );
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        if ( is_home() && get_query_var( 'paged' ) <= 0 ) {
            while ( have_posts() ) {
                ...
                $width = 'on' === $fullwidth ? 1080 : 750;
                $width = (int) apply_filters( 'et_pb_blog_image_width', $width );

                $height = 'on' === $fullwidth ? 675 : 355;
                ...
                <?php
                    ...
                    if ( ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) {
                        ...
                        elseif ( '' !== $thumb && 'on' === $show_thumbnail ) :
                            if ( 'on' !== $fullwidth ) echo '<div class="featured__image">'; ?>
                                <a href="<?php esc_url( the_permalink() ); ?>" class="f__no-und">
                                ...
                                <?php
                            if ( 'on' !== $fullwidth ) echo '</div> <!-- .hero__image -->';
                        endif;
                    }
                ?>

                <div class="">
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'featured__inner'); ?>>
                            ...
                            <div class="post__media-content">
                                <?php
                                    if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_categories || 'on' === $show_comments ) {
                                        printf( '<div>%1$s %2$s %3$s %4$s %5$s %6$s %7$s</div>',
                                        (
                                            'on' === $show_author
                                            ? sprintf( __( '%s', 'et_builder' ), '<div class="f__up f__no-und categories__link">' . et_pb_get_the_author_posts_link() . '</div>' )
                                            : ''
                                        ),
                                        (
                                            ( 'on' === $show_author && 'on' === $show_date )
                                            ? ' <span class="categories__separator">|</span> '
                                            : ''
                                        ),
                                        (
                                            'on' === $show_date
                                        ...
                                        (
                                            'on' === $show_categories
                                            ? sprintf(__('%s', 'et_builder'), '<div class="f__up categories__link">' . strip_tags(get_the_category_list(' | ') . '</div>' ) )
                                            : ''
                                        ),
                                        (
                        ...
                        <h2 class="featured__title c__black"><a href="<?php esc_url( the_permalink() ); ?>" class="f__no-und"><?php the_title(); ?></a></h2>
            ...
            if ( 'on' !== $fullwidth )
            $output = sprintf( '<div class="featured__news">%1$s</div>', $output );

            return $output;
        }
    }
}
new DS_Custom_Featured_Blog;
